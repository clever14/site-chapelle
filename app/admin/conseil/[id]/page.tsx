...
import { modifierConseil } from "@/lib/actions";
import { notFound } from "next/navigation";

export default async function EditConseil({ params }: { params: { id: string } }) {
  const items = await getConseil();
  const m = items.find((x) => x.id === params.id);
  if (!m) notFound();

  return (
    <div className="mx-auto max-w-2xl">
      <Link href="/admin/conseil" className="mb-4 inline-block text-[14px] text-attenue2 hover:text-marial">← Retour</Link>
      <h1 className="mb-6 font-display text-[30px] font-semibold text-marial">Modifier le membre</h1>

      <form action={modifierConseil} className="card space-y-4 p-6">
