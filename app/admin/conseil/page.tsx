...
export default async function AdminConseil() {
  const membres = await getConseil();
  return (
    <div>
      <h1 className="mb-8 font-display text-[30px] font-semibold text-marial">Conseil paroissial</h1>
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        <form action={creerConseil} className="card h-fit space-y-4 p-6">
          <h2 className="font-display text-[20px] font-semibold text-encre">Ajouter un membre du conseil</h2>
          <input name="nom" required placeholder="Nom (ex. M. Koffi Bernard)" className="w-full rounded-field border border-bord-2 bg-carte px-4 py-2.5 outline-none focus:border-or" />
          <input name="fonction" placeholder="Fonction (ex. Président, Secrétaire, Trésorier…)" className="w-full rounded-field border border-bord-2 bg-carte px-4 py-2.5 outline-none focus:border-or" />
          <div>
